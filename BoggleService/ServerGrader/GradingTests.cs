...
namespace ServerGrader
{
    ...
    public class GradingTests
    {
        ...
        private async Task<string> MakeUser(String nickname, HttpStatusCode status = 0)
        {
            ...
            {
                HttpResponseMessage response = await client.PostAsync("/BoggleService/users", Serialize(nickname));
        ...
        /// Returns a Task that will produce the resulting object.
        /// </summary>
        private async Task<dynamic> JoinGame(String player, int timeLimit, HttpStatusCode status = 0)
        {
            ...
            using (HttpClient client = CreateClient())
            {
                HttpResponseMessage response = await client.PostAsync("/BoggleService/games", Serialize(user));
                if (status != 0) Assert.AreEqual(status, response.StatusCode);
                if (response.IsSuccessStatusCode)
                {
                    String result = await response.Content.ReadAsStringAsync();
        ...
        private async Task CancelGame(String player, HttpStatusCode status = 0)
        {
            using (HttpClient client = CreateClient())
            {
                HttpResponseMessage response = await client.PutAsync("/BoggleService/games", Serialize(player));
                if (status != 0) Assert.AreEqual(status, response.StatusCode);
            }
        }
        ...
        /// </summary>
        private async Task<dynamic> GetStatus(String game, bool brief, HttpStatusCode status = 0)
        {
            using (HttpClient client = CreateClient())
            {
                HttpResponseMessage response = await client.GetAsync("/BoggleService/games/" + game + "/" + brief);
                if (status != 0) Assert.AreEqual(status, response.StatusCode);
                if (response.IsSuccessStatusCode)
                {
                    String result = await response.Content.ReadAsStringAsync();
        ...
        private async Task<int> PlayWord(String player, String game, String word, HttpStatusCode status = 0)
        {
            ...
            using (HttpClient client = CreateClient())
            {
                HttpResponseMessage response = await client.PutAsync("/BoggleService/games/" + game, Serialize(play));
                if (status != 0) Assert.AreEqual(status, response.StatusCode);
                if (response.IsSuccessStatusCode)
                {
                    String result = await response.Content.ReadAsStringAsync();
        ...
        public void Reset()
        {
            ...
            if ((bool) JoinGame(player1, 10).Result.IsPending)
            {
        ...
        // First player to join results in pending game
        [TestMethod]
        public void JoinGame4()
        {
            Reset();
            String player1 = MakeUser("Player 1").Result;
            Assert.IsTrue((bool) JoinGame(player1, 10, OK).Result.IsPending);
        }
        ...
        public void JoinGame5()
        {
            Reset();
            String player1 = MakeUser("Player 1").Result;
            dynamic game = JoinGame(player1, 10).Result;
            Assert.IsTrue(game.GameID.ToString().Length > 0);
        }
        ...
        // Two players joining starts a game
        [TestMethod]
        public void JoinGame7()
        {
            ...
            JoinGame(player1, 10).Wait();
            Assert.IsFalse((bool)JoinGame(player2, 20, OK).Result.IsPending);
        }
        ...
        public void JoinGame8()
        {
            ...
            String player2 = MakeUser("Player 2").Result;
            JoinGame(player1, 10).Wait();
            Assert.IsTrue(JoinGame(player2, 20).Result.GameID.ToString().Length > 0);
        }
        ...
        public void JoinGame9()
        {
            ...
            String player1 = MakeUser("Player 1").Result;
            String player2 = MakeUser("Player 2").Result;
            dynamic game1 = JoinGame(player1, 10).Result;
            dynamic game2 = JoinGame(player2, 20).Result;
        ...
        public void JoinGame10()
        {
            ...
            String player2 = MakeUser("Player 2").Result;
            JoinGame(player1, 10).Wait();
            dynamic game1 = JoinGame(player2, 20).Result;

            String player3 = MakeUser("Player 3").Result;
            String player4 = MakeUser("Player 4").Result;
            JoinGame(player3, 10).Wait();
            dynamic game2 = JoinGame(player4, 20).Result;

            Assert.AreNotEqual(game1.GameID, game2.gameID);
        }
        ...
        public void TestCancelGame5()
        {
            ...
            String player1 = MakeUser("Player 1").Result;
            JoinGame(player1, 10).Wait();
            CancelGame(player1, NoContent).Wait();
        }
        ...
        public void TestPlayWord1()
        {
            Reset();
            string player1 = MakeUser("Player 1").Result;
            string game1 = JoinGame(player1, 10).Result.GameID;
        ...
        public void TestPlayWord2()
        {
            ...
            string game2 = JoinGame(player2, 6).Result.GameID;
            Thread.Sleep(7000);
        ...
        public void TestPlayWord15()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 10).Result.GameID;
            string board = GetStatus(game2, false).Result.Board;
        ...
        {
            GetStatus("xyzzy123", false, Forbidden).Wait();
        }
        ...
        public void TestStatus2()
        {
            ...
            string player1 = MakeUser("Player 1").Result;
            string game1 = JoinGame(player1, 10).Result.GameID;
            dynamic status = GetStatus(game1, false).Result;
        ...
        public void TestStatus3()
        {
            ...
            string player2 = MakeUser("Player 2").Result;
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            string state = GetStatus(game2, false).Result.GameState;
        ...
        public void TestStatus5()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            int limit = GetStatus(game2, false).Result.TimeLimit;
        ...
        public void TestStatus7()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            string name1 = GetStatus(game2, false).Result.Player1.Nickname;
        ...
        public void TestStatus8()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            string name2 = GetStatus(game2, false).Result.Player2.Nickname;
        ...
        public void TestStatus9()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            int score1 = GetStatus(game2, false).Result.Player1.Score;
        ...
        public void TestStatus10()
        {
            ...
            JoinGame(player1, 10).Wait();
            string game2 = JoinGame(player2, 20).Result.GameID;
            int score2 = GetStatus(game2, false).Result.Player2.Score;
        ...
        public void TestStatus17()
        {
            ...
            Thread.Sleep(7000);
            int score1 = GetStatus(game2, false).Result.Player2.Score;
            Assert.AreEqual(-1 + GetScore(words[0]) + GetScore(words[1]), score1);
        }
        ...
        public void TestStatus18()
        {
            ...
            Thread.Sleep(7000);
            List<dynamic> wordscores = new List<dynamic>(GetStatus(game2, false).Result.Player1.WordsPlayed);
        ...
        public void TestStatus19()
        {
            ...
            Thread.Sleep(7000);
            List<dynamic> wordscores = new List<dynamic>(GetStatus(game2, false).Result.Player2.WordsPlayed);
            wordscores.Sort((x, y) => x.Word.CompareTo(y.Word));
            allWords.Add("XYZZY");
            allWords.Add("IS");
            allWords.Sort();
        ...
        public string SimulateGame(out List<string> p1Words, out List<string> p2Words, AutoResetEvent resetEvent)
        {
            ...
            string game2 = JoinGame(player2, 30).Result.GameID;
        ...
        public void Play(AutoResetEvent resetEvent)
        {
            ...
            string board = GetStatus(game, false).Result.Board;
            List<dynamic> wordscores1 = new List<dynamic>(GetStatus(game, false).Result.Player1.WordsPlayed);
            List<dynamic> wordscores2 = new List<dynamic>(GetStatus(game, false).Result.Player2.WordsPlayed);
            ...
            int score1 = GetStatus(game, false).Result.Player1.Score;
            int score2 = GetStatus(game, false).Result.Player2.Score;
        ...
        /// </summary>
        private void CheckStatus(string game, string state, bool brief, string p1, string p2, string n1, string n2, string b,
