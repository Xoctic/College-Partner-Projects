...
namespace Formulas
{
    ...
    public class Formula
    {
        ...
        {
            List<Token> tokenCheck = new List<Token>(GetTokens(formula));
            ...
            //condition 5: The first token of a formula must be a number, a variable, or an opening parenthesis.
            else if (tokenCheck[0].Type == TokenType.Oper || tokenCheck[0].Type == TokenType.RParen || tokenCheck[0].Type == TokenType.Invalid)
            {
                throw new FormulaFormatException(tokenCheck[0].Text);
            }


            foreach (Token token in tokenCheck)
            {
            ...
            //condition 6: The last token of a formula must be a number, a variable, or a closing parenthesis.
            if (tokenCheck[tokenCheck.Count - 1].Type == TokenType.Oper || tokenCheck[tokenCheck.Count - 1].Type == TokenType.LParen || tokenCheck[tokenCheck.Count - 1].Type == TokenType.Invalid)
            {
                throw new FormulaFormatException(tokenCheck[tokenCheck.Count - 1].Text);
            }
            //condition 4: The total number of opening parentheses must equal the total number of closing parentheses.
            if (numOfLeftParen != numOfRightParen)
            {
